...
statisiques::statisiques(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::statisiques)
{
    ui->setupUi(this);
    ...
    QSqlQuery q;
    q.prepare("SELECT nom_marque,count(nom_marque) nbr FROM machine GROUP BY nom_marque ")  ;
    if(q.exec()){
    while (q.next()) {
    ...
    ui->widget->xAxis->setLabel("marque");
...
void statisiques::on_home_button_clicked()
{
  menu m;
this->hide();
m.setModal(true);
